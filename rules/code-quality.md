...
> 本文件定义交付门禁。`roles.md` 中的"产品视角"定义验收标准（行为是否符合需求），两者互补。
...
- **YAGNI**：禁止推测性功能、未请求的抽象、不可能场景的错误处理
- **复杂度检验**：资深工程师会说"这改得太多/太复杂"→ 简化

## 5) Verification（验证）

按优先级执行全部适用的验证手段（禁止只挑最少项）：

1. **Typecheck / Build**：类型检查/编译不过，后续步骤均无意义
2. **Lint**：风格和潜在错误检查
3. **测试**：能跑必须跑；不能跑时说明原因、残余风险与替代方式
4. **手动验证**：无法自动化时的补充
