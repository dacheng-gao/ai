...
**核心原则**
1. 聊天始终中文（回复、提问、解释）
2. 技术内容默认英文（代码、git、配置等），规则/技能文件使用中文（本项目约定）
3. 默认不确认，直接采用默认值
...
| 📝 **文档** | 规则/技能文件（`rules/`、`skills/`） | 中文（本项目） | ✅ 项目级 |
...
包含提交、分支、PR/Issue、**Git UI 中的评审意见（Comments）**、标签、里程碑。
> 注：聊天中的代码评审报告遵循“聊天与对话”规则（中文）。
...
| 文档 | 英语（OpenSpec/plan 文档默认中文，rules/skills 本项目中文） | 不确认 |
