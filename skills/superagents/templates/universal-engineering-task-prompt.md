...
2. 输出：`目标/范围/不做/关键假设`。
3. 在继续前做最小目标校验：检查是否目标模糊、疑似 XY 问题、或当前路径明显非最优。
4. `Lite` 且需求明确：可 1-2 句回显并继续。
5. `Standard/Full` 且需求明确、实现路径唯一：开场对齐后继续执行。
6. 仅在业务决策、缺少关键输入、明显歧义、多方案权衡、目标模糊或疑似 XY 问题时提问并等待用户确认。
7. 需求模糊、跨模块交互或多方案分歧：调用 `superpowers:brainstorming` 收敛边界后再进入模板 B。
8. 已进入 `superpowers:brainstorming` 时，提问改为“一次一问”。
9. `AskUserQuestion` 不可用时，改为普通文本提问并保持 `proceed: wait_for_user`。
...
输出结构：
1. `直接执行`：一句话说明是否完成，并给出请求对照、关键改动、验证证据
2. `深度交互`：只做最小必要挑战，检查是否存在 XY 问题、当前路径代价、或更低成本替代方案
3. 若无有效挑战点，明确说明当前路径合理，暂无更优替代建议
4. 残余风险：未完成项与影响
5. 下一步：最多 3 条
