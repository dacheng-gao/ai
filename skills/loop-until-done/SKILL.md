...
**默认触发：** 几乎所有需要满足用户需求的任务（回答、分析、代码、文档、解释、建议等）

**仅不触发：** 极其简单的单步操作（如"2+2"）、纯粹问候、用户明确要求"简单回答即可"的低风险查询

> **核心原则：** 有用户需求就迭代。一次回答往往有理解偏差；迭代确保准确和完整。
...
1. **参考 `rules/roles.md`**：10 个可用角色、任务-角色映射表、输出格式规范
2. **执行角色分析**：选择角色 → 按角色视角分析 → 综合输出
3. **关键规则**：至少两个视角、角色深度匹配、代码变更必须包含语言专家、用户指定角色优先
